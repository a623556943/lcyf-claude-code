...
description: 提供从开发过程中自动学习和积累知识的能力。当执行 /lcyf-learn 命令、会话结束或重要操作完成后激活。包含模式提取、知识存储、本能构建功能。
---
...
### 核心内容
- **学习机制流程**（第13-28行）：
  - 学习流程图：开发活动 → 模式识别 → 质量评估 → 知识存储 → 本能升级
  - 4种学习来源：代码变更、问题解决、代码审查、测试用例

- **4种知识类型定义**（第29-102行）：
  - 代码模式 (code-patterns)：可复用的代码片段（第31-53行）
  - 问题解决方案 (problem-solutions)：问题-解决方案对（第54-74行）
  - 团队约定 (conventions)：编码规范和约定（第75-85行）
  - 本能规则 (instincts)：高频模式自动升级（第87-102行）

- **知识库结构**（第104-125行）：
  - 目录树形图：build-parent/doc/knowledge/ 下的完整目录结构
  - learned-patterns, team-conventions, instincts, module-knowledge 分类

- **模式提取流程**（第148-182行）：
  - 6步流程：监听操作 → 提取模式 → 评估质量 → 用户确认 → 存储知识 → 本能升级
  - 流程图详细说明

- **学习评估标准**（第127-147行）：
  - 模式质量评估：频率≥3次，置信度≥0.8
  - 本能升级条件：频率≥10次，置信度≥0.95
...
- **知识管理命令**（第207-223行）：
  - list, search, add, export, cleanup 命令说明
